...
void testAnalogRead()
{ for(int i=0; i<testsamples; i++) slow10test.addValue(analogRead(adcPin));
  for(int i=0; i<testsamples; i++) fast10test.addValue(analogReadFast(adcPin));
#if defined(__arm__) 
  analogReadResolution(12); 
  for(int i=0; i<testsamples; i++) slow12test.addValue(analogRead(adcPin));
  for(int i=0; i<testsamples; i++) fast12test.addValue(analogReadFast(adcPin));
...
{ Serial.begin(9600);
  ...
  printAll();
  if(doprintExecutionTime) printExecutionTime();
  if(doPrintStandardDeviation) printStandardDeviation();
}
